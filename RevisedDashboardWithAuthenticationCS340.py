...
###########################
# Set up authentication for username and password credentials

# Bring in hashlib module to create secure hashes and SHA_256 for security
import hashlib

User_authentication = {
    "username": hashlib.sha256("password".encode()).hexdigest()  # hashed password to securely store user credentials
}

# Function to validate user authentication
def validate_user(username, password):
    hashed_password = hashlib.sha256(password.encode()).hexdigest()
    return User_authentication.get(username) == hashed_password
...
# Login fields for the Start Page
login.layout = html.Div([
    html.H2("Please Login Here to Begin"),   # message to direct user to login to proceed
    dcc.Input(id='username', type='text', placeholder='Username'),
    dcc.Input(id='password', type='password', placeholder='Password'),
    html.Button('Login', id='login-button'), # create button  with verbiage "Login" for users
    html.Div(id='login-output', labelStyle={'color': 'red'})  # utilize red color to stand out
...
# Interaction for Login Page
    @app.callback(
        Output('login-output', 'children'),  # property of the component with the ID login-output
        Output('login-output', 'id'),        # identify dash components in callbacks
        Input('login-button', 'n_clicks'),   # interactive button clicks
        State('username', 'value'),          # the value is the username entered
        State('password', 'value')           # the value is the password entered
        )
    def login(n_clicks, username, password):
        if n_clicks is None:
            return "", 'login-output'
# Successful login user can connect to the user interface and Database 
